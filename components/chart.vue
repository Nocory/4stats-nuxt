...
//const chartFunctions = require("~/assets/js/chartFunctions")
export default {
	data: () => ({
		chartOptions : {
			term: "day",
	...
	methods: {
		...
		toggleBoard(board) {
			...
			if(index >= 0){
				...
				this.$chartFunctions.removeBoard(board)
			}else{
		...
		checkIfRequestOrAdd(board){
			...
			if (Date.now() > validUntil) {
				...
				this.$chartFunctions.addBoard(board,timelineData.history,this.chartOptions)
			}
		},
		...
		processNewTimelineData(board, term, newData) {
			...
			this.$chartFunctions.addBoard(board,historyData.history,this.chartOptions)
		}
	},
	mounted() {
		if(process.browser){
			this.$chartFunctions.init("myChart")
