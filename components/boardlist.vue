  ...
  <div class="boardlist-component">
    ...
    <div class="boardlist-wrapper">
      <div class="boardlist-header board-data-wrapper">
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=name`" class="board-data board-name" :class="{'category-selected' : sortBoardListBy == 'name'}">Board</nuxt-link>
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=postsPerMinute`" class="board-data" :class="{'category-selected' : sortBoardListBy == 'postsPerMinute'}" data-hover-text="Over the last hour">Posts/min</nuxt-link>
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=threadsPerHour`" class="board-data is-hidden-touch" :class="{'category-selected' : sortBoardListBy == 'threadsPerHour'}" data-hover-text="Over the last hour">Threads/hour</nuxt-link>
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=avgPostsPerDay`" class="board-data" :class="{'category-selected' : sortBoardListBy == 'avgPostsPerDay'}" data-hover-text="Over the last 4 weeks. Weighted towards more recent weeks.">Avg.Posts/day</nuxt-link>
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=imagesPerReply`" class="board-data is-hidden-touch is-hidden-desktop-only" :class="{'category-selected' : sortBoardListBy == 'imagesPerReply'}" data-hover-text="Posts with files attached">Images</nuxt-link>
        <nuxt-link :to="`/?board=${selectedBoard}&sortBy=relativeActivity`" class="board-data" :class="{'category-selected' : sortBoardListBy == 'relativeActivity'}" data-hover-text="Current posts-per-minute relative to the boards usual top ppm rate">Activity Now</nuxt-link>
      </div>
      <transition-group name="flip-list" class="board-rows" tag="div">
        <div class="board-row" v-for="boardName in sortedBoardlist" :key="boardName">
          <nuxt-link :to="`/?board=${boardName}&sortBy=${sortBoardListBy}`" :id="'board-'+boardName" class="board-data-wrapper" :class="{'board-selected' : (selectedBoard == boardName)}">
...
export default {
	data: () => ({
		longBoardNames : require('~/assets/js/config').default.boardNames
	}),
	computed: {
		...vuex.mapState([
			"boardData",
